...
// This will true when AI will take time to generate response
...
const onSubmit=async(e)=>{
  ...
  try{
     e.preventDefault();
     if(!user) return toast('Please login to send message');
     setLoading(true);

     const promptCopy=prompt;
     setPrompt('');
     setMessage(prev=>[...prev,{role:'user',content:prompt,timestamp
...
useEffect(()=>{
  ...
  return (
    <div className='flex-1 flex flex-col justify-between m-5 nd:m-10 xl:mx-30
      ...
      {/* prompt input box */}
      {/* Now create input box where user will type the input */}
